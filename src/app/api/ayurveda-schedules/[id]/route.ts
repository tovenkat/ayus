...
async function ownedOr404(userId: string, id: string) {
  const row = await prisma.ayurvedicSchedule.findUnique({ where: { id }, select: { userId: true } });
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  const row = await prisma.ayurvedicSchedule.update({ where: { id }, data });
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  await prisma.ayurvedicSchedule.delete({ where: { id } });
  for (const p of ["/ayurveda", "/dashboard"]) revalidatePath(p);
